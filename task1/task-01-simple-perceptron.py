...
class Perceptron:
    ...
    def fit(self, X, y):
        ...
        #Online learning loop for the Perceptron algorithm
        for epoch in range(self.epochs):

            #Iterate through each sample and its corresponding target label
            for xi, target in zip(X, y):

                #predict the output for the current input sample
                y_pred = self.predict(xi)[0]

                #calculate the error between the predicted and actual target
                error = target - y_pred

                if error != 0:
                    #weight updates using the Perceptron learning rule
                    self.weights += self.learning_rate * error * xi

                    #bias updates using the Perceptron learning rule
